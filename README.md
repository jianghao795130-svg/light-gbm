...
以下结果摘自当前项目目录下的 `research_outputs` 输出文件，属于一次具体研究配置下的样本外评估快照，主要用于展示研究链路的可运行性与结果口径，不应视为稳定可复制的未来收益承诺。

### 1. IC 指标

来自 `research_outputs/ic_summary.csv` 的当前结果如下：

| 指标 | 均值 | 标准差 | IR | 胜率 |
| --- | ---: | ---: | ---: | ---: |
| Rank IC (Spearman) | 0.0749 | 0.1354 | 0.5533 | 72.42% |
| Normal IC (Pearson) | 0.0496 | 0.1138 | 0.4362 | 68.53% |

这表明当前 `ml_factor` 在截面排序层面具备一定稳定性，其中 Rank IC 显著高于 Pearson IC，也符合股票横截面排序任务通常更关注相对次序而非绝对收益拟合的研究习惯。

### 2. TopN 组合样本外表现

来自 `research_outputs/topn_performance.csv` 的当前结果如下：

| 组合 | 累计收益 | 年化收益 | 年化波动 | Sharpe | 最大回撤 | 平均买入换手 | 平均卖出换手 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Top50 | 196.15% | 22.96% | 38.05% | 0.60 | -28.28% | 49.94% | 48.42% |
| Top100 | 176.98% | 21.41% | 32.20% | 0.66 | -27.22% | 37.24% | 35.73% |
| Top150 | 160.63% | 20.01% | 29.88% | 0.67 | -26.59% | 29.43% | 27.92% |
| Top200 | 146.53% | 18.74% | 28.40% | 0.66 | -25.25% | 22.80% | 21.28% |
| 全市场等权 | 53.46% | 8.50% | 24.50% | 0.35 | -31.42% | N/A | N/A |

从当前结果看，组合集中度越高，收益弹性越强，但换手与波动也同步抬升；Top100 至 Top200 在收益、回撤和交易摩擦之间呈现出更均衡的研究特征。

### 3. TopN / TopM 降换手效果

来自 `research_outputs/topm_buffer_search.csv` 的最优几组结果显示：

| TopM 倍数 | TopM 映射示例 | 每个平均持仓周期降低的换手率 | 平均净收益保留率 |
| --- | --- | ---: | ---: |
| 10.0 | `{50: 500, 100: 1000, 150: 1500, 200: 2000}` | 44.05% | 123.32% |
| 8.0 | `{50: 400, 100: 800, 150: 1200, 200: 1600}` | 39.03% | 121.98% |
| 6.0 | `{50: 300, 100: 600, 150: 900, 200: 1200}` | 33.31% | 120.39% |

这说明在当前实验样本上，引入保留池缓冲机制后，换手率存在明显压降空间，而且并未观察到净收益同步恶化。实际使用时仍建议增加收益保留、回撤约束和容量约束，避免单目标寻参造成结果失真。

### 4. 当前主要特征

来自 `research_outputs/feature_importance_ranking.csv` 的综合排名靠前特征包括：
...
从当前结果看，模型主要由短中期价格行为、交易活跃度、回撤状态与部分估值信息共同驱动，整体上符合 A 股横截面多因子选股中“量价 + 反转 + 交易拥挤度 + 基本面补充”的常见研究图景。
